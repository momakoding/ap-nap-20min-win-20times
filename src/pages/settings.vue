...
import { useSettings, useLocale, type Language } from '@/composables'
...
<template>
  <div class="settings">
    <h2 class="settings__title">{{ lang === 'en' ? 'Settings' : '设置' }}</h2>

    <div class="settings__panel">
      <!-- Audio -->
      <section class="settings__section">
        <h3 class="settings__section-title">{{ lang === 'en' ? 'Audio' : '音频' }}</h3>

        <div class="settings__row">
          <label class="settings__label">{{ lang === 'en' ? 'Mute' : '静音' }}</label>
          <button
        ...
        <div class="settings__row" :class="{ 'settings__row--disabled': settings.muted }">
          <label class="settings__label">{{ lang === 'en' ? 'BGM Volume' : '背景音乐' }}</label>
        ...
        <div class="settings__row" :class="{ 'settings__row--disabled': settings.muted }">
          <label class="settings__label">{{ lang === 'en' ? 'SFX Volume' : '音效' }}</label>
          <div class="settings__slider-wrap">
            <input
      ...
      <!-- Language -->
      <section class="settings__section">
        <h3 class="settings__section-title">{{ lang === 'en' ? 'Language / 语言' : '语言 / Language' }}</h3>
        <div class="settings__row">
          <label class="settings__label">{{ lang === 'en' ? 'Interface' : '界面语言' }}</label>
    ...
    <GameButton :label="lang === 'en' ? 'Back to Home' : '返回主页'" variant="secondary" @click="router.push({ name: 'home' })" />
  </div>
</template>
